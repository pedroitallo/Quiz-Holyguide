/*
  # Create step_views_fn1_org table
  ...
  2. Security
    - Enable RLS on `step_views_fn1_org` table
...
CREATE TABLE IF NOT EXISTS step_views_fn1_org (
...
ALTER TABLE step_views_fn1_org ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow anon to insert step_views_fn1_org"
  ON step_views_fn1_org
  FOR INSERT
  TO anon
  WITH CHECK (true);

CREATE POLICY "Allow anon to update step_views_fn1_org"
  ON step_views_fn1_org
  FOR UPDATE
  TO anon
  USING (true)
...
CREATE POLICY "Allow anon to select step_views_fn1_org"
  ON step_views_fn1_org
  FOR SELECT
  TO anon
  USING (true);

CREATE INDEX IF NOT EXISTS idx_step_views_fn1_org_session_id ON step_views_fn1_org(session_id);
CREATE INDEX IF NOT EXISTS idx_step_views_fn1_org_ab_test_id ON step_views_fn1_org(ab_test_id);
CREATE INDEX IF NOT EXISTS idx_step_views_fn1_org_created_at ON step_views_fn1_org(created_at);
