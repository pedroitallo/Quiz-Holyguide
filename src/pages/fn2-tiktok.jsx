...
import PaywallStep from "../components/quiz/fn2-tiktok/PaywallStep";
...
export default function Fn2TiktokPage() {
  ...
  useEffect(() => {
    if (currentStep < 16) {
      ...
      localStorage.setItem('holymind_quiz_state_fn2_tiktok', JSON.stringify(stateToSave));
    } else if (currentStep === 17) {
      localStorage.removeItem('holymind_quiz_state_fn2_tiktok');
    }
  }, [currentStep, formData]);

  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_fn2_tiktok');
      if (savedStateJSON) {
        try {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('fn2-tiktok', stepNames[currentStep - 1]);
    }
  }, [currentStep]);
