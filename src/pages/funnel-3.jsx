...
import InitiateQuiz from "../components/quiz/funnel-3/InitiateQuiz";
import TestimonialsCarousel from "../components/quiz/shared/TestimonialsCarousel";
import BirthDateWithZodiac from "../components/quiz/funnel-3/BirthDateWithZodiac";
import LoveSituationStep from "../components/quiz/shared/LoveSituationStep";
import IdealPartnerQualities from "../components/quiz/funnel-3/IdealPartnerQualities";
import PartnerPreference from "../components/quiz/funnel-3/PartnerPreference";
import BirthChartResults from "../components/quiz/funnel-3/BirthChartResults";
import LoveChallenge from "../components/quiz/funnel-3/LoveChallenge";
import LoveDesire from "../components/quiz/funnel-3/LoveDesire";
import SoulmateConnection from "../components/quiz/funnel-3/SoulmateConnection";
import LoveLanguage from "../components/quiz/funnel-3/LoveLanguage";
import RelationshipEnergy from "../components/quiz/funnel-3/RelationshipEnergy";
import FutureScenario from "../components/quiz/funnel-3/FutureScenario";
import LoadingRevelation from "../components/quiz/shared/LoadingRevelation";
import SoulmateDrawingLoading from "../components/quiz/funnel-3/SoulmateDrawingLoading";
...
export default function Funnel3Page() {
  ...
  useEffect(() => {
    if (currentStep < 16) {
      ...
      localStorage.setItem('holymind_quiz_state_funnel3', JSON.stringify(stateToSave));
    } else if (currentStep === 17) {
      localStorage.removeItem('holymind_quiz_state_funnel3');
    }
  }, [currentStep, formData]);

  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_funnel3');
      if (savedStateJSON) {
        try {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('funnel-3', stepNames[currentStep - 1]);
    }
  }, [currentStep]);
