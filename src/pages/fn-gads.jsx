...
export default function FnGadsPage() {
      ...
      try {
        ...
        if (typeof window !== 'undefined' && window.rtgNoBack) {
          try {
            ...
            console.log('✅ RedTrack StartQuiz event fired');
          } catch (error) {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('fn-gads', stepNames[currentStep - 1]);
    }

    // RedTrack: EndQuiz Event when reaching Paywall step (step 8)
    if (currentStep === 8) {
      if (typeof window !== 'undefined' && window.rtgNoBack) {
        try {
          window.rtgNoBack('event', 'EndQuiz');
          console.log('✅ RedTrack EndQuiz event fired');
        } catch (error) {
