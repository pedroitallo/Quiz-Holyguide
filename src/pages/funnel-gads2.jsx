...
import PaywallStep from "../components/quiz/funnel-gads2/PaywallStep";
...
export default function FunnelGads2Page() {
  ...
  useEffect(() => {
    if (currentStep < 16) {
      ...
      localStorage.setItem('holymind_quiz_state_funnel_gads2', JSON.stringify(stateToSave));
  ...
  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_funnel_gads2');
      if (savedStateJSON) {
        try {
