...
import PaywallStep from "../components/quiz/funnel-2.1/PaywallStep";
import ThankYouStep from "../components/quiz/shared/ThankYouStep";

export default function Funnel21Page() {
  ...
  useEffect(() => {
    if (currentStep < 16) {
      ...
      localStorage.setItem('holymind_quiz_state_funnel21', JSON.stringify(stateToSave));
    } else if (currentStep === 17) {
      localStorage.removeItem('holymind_quiz_state_funnel21');
    }
  }, [currentStep, formData]);

  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_funnel21');
      if (savedStateJSON) {
        try {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('funnel-2.1', stepNames[currentStep - 1]);
    }
  }, [currentStep]);
