...
export default function Funnel2AffPage() {
  ...
  useEffect(() => {
    if (currentStep < 17) {
      ...
      localStorage.setItem('holymind_quiz_state_funnel2aff', JSON.stringify(stateToSave));
    } else if (currentStep === 18) {
      localStorage.removeItem('holymind_quiz_state_funnel2aff');
    }
  }, [currentStep, formData]);

  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_funnel2aff');
      if (savedStateJSON) {
        try {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('funnel-2aff', stepNames[currentStep - 1]);
    }
  }, [currentStep]);
