...
export default function FunnelVsl1Page() {
  ...
  useEffect(() => {
    if (currentStep < 9) {
        ...
        localStorage.setItem('holymind_quiz_state_vsl1', JSON.stringify(stateToSave));
  ...
  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_vsl1');
      if (savedStateJSON) {
          try {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('funnel-vsl1', stepNames[currentStep - 1]);
    }
  }, [currentStep]);
