...
export default function FunnelTtPage() {
  ...
  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_funnel_tt');
      if (savedStateJSON) {
          try {
      ...
      try {
        console.log('🚀 Creating new quiz session (funnel-tt)...');
        ...
        console.log('📊 UTM Parameters (funnel-tt):', { utmSource, utmMedium, utmCampaign, src });
        
        const newQuizResult = await HybridQuizResult.create({
          funnel_type: 'funnel-tt',
        ...
        console.log('✅ New QuizResult created successfully (funnel-tt):', newQuizResult.id);
      } catch (error) {
        console.error('❌ CRITICAL: Failed to create QuizResult (funnel-tt), using offline mode:', error.message, error);
        ...
        <div className="px-4 md:px-6 py-3">
          <div className="flex items-center gap-3">
            <div className="relative">
              <img src="https://reoszoosrzwlrzkasube.supabase.co/storage/v1/object/public/user-uploads/images/1759890624957-jkxekrn97yd.png" alt="Madame Aura" className="w-10 md:w-12 h-10 md:h-12 rounded-full object-cover border-2 border-purple-200" />
