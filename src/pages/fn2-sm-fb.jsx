...
import PaywallStep from "../components/quiz/fn2-sm-fb/PaywallStep";
import ThankYouStep from "../components/quiz/shared/ThankYouStep";

export default function Fn2SmFb() {
  ...
  useEffect(() => {
    if (currentStep < 16) {
      ...
      localStorage.setItem('holymind_quiz_state_fn2_sm_fb', JSON.stringify(stateToSave));
    } else if (currentStep === 17) {
      localStorage.removeItem('holymind_quiz_state_fn2_sm_fb');
    }
  }, [currentStep, formData]);

  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_fn2_sm_fb');
      if (savedStateJSON) {
        try {
      ...
      try {
        ...
        if (window.RedTrack) {
          window.RedTrack('event', 'start_quiz');
          console.log('RedTrack: start_quiz event sent');
        }

        const newQuizResult = await HybridQuizResult.create({
          funnel_type: 'fn2-sm-fb',
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('fn2-sm-fb', stepNames[currentStep - 1]);
    }
  }, [currentStep]);
