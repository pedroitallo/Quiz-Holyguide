...
import PaywallStep from "../components/quiz/fn-shorts2/PaywallStep";
...
export default function FnShortVsl1Page() {
  ...
  useEffect(() => {
    if (currentStep < 9) {
        ...
        localStorage.setItem('holymind_quiz_state_fnshort_vsl1', JSON.stringify(stateToSave));
    } else if (currentStep === 9) {
        localStorage.removeItem('holymind_quiz_state_fnshort_vsl1');
    }
  }, [currentStep, formData]);

  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_fnshort_vsl1');
      if (savedStateJSON) {
          try {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('fnshort-vsl1', stepNames[currentStep - 1]);
    }
  }, [currentStep]);
  ...
  return (
    <div className="min-h-screen bg-[#f9f5ff] relative overflow-hidden" style={{ userSelect: 'none' }}>
      ...
      <div className={`bg-[#f9f5ff] pb-8 px-2 md:px-4 ${currentStep === 8 ? 'pt-8' : 'pt-24 md:pt-28'}`}>
