...
import PaywallStep from "../components/quiz/fn1-dsv/PaywallStep";
...
export default function Fn1DsvPage() {
  ...
  useEffect(() => {
    const initializeQuizSession = async () => {
      ...
      try {
        console.log('🚀 Creating new quiz session for fn1-dsv...');
        ...
        if (typeof window !== 'undefined' && window.rtgNoBack) {
          try {
            ...
            console.log('✅ RedTrack StartQuiz event fired for fn1-dsv');
          } catch (error) {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('fn1-dsv', stepNames[currentStep - 1]);
    }
    ...
    if (currentStep === 8) {
      if (typeof window !== 'undefined' && window.rtgNoBack) {
        try {
          ...
          console.log('✅ RedTrack EndQuiz event fired for fn1-dsv');
        } catch (error) {
  ...
  return (
    <VisualEditorProvider funnelId="fn1-dsv" stepSlug={`step-${currentStep}`}>
      <VisualEditorWrapper>
