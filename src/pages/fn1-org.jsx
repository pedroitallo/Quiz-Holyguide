...
import PaywallStep from "../components/quiz/fn1-org/PaywallStep";
...
export default function Fn1OrgPage() {
  ...
  useEffect(() => {
    const initializeQuizSession = async () => {
      ...
      try {
        console.log('🚀 Creating new quiz session for fn1-org...');
        ...
        if (typeof window !== 'undefined' && window.rtgNoBack) {
          try {
            ...
            console.log('✅ RedTrack StartQuiz event fired for fn1-org');
          } catch (error) {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('fn1-org', stepNames[currentStep - 1]);
    }
    ...
    if (currentStep === 8) {
      if (typeof window !== 'undefined' && window.rtgNoBack) {
        try {
          ...
          console.log('✅ RedTrack EndQuiz event fired for fn1-org');
        } catch (error) {
  ...
  return (
    <VisualEditorProvider funnelId="fn1-org" stepSlug={`step-${currentStep}`}>
      <VisualEditorWrapper>
