...
export default function FnShorts2Page() {
  ...
  useEffect(() => {
    if (currentStep < 16) {
      ...
      localStorage.setItem('holymind_quiz_state_fn_shorts2', JSON.stringify(stateToSave));
  ...
  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_fn_shorts2');
      if (savedStateJSON) {
        try {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('fn-shorts2', stepNames[currentStep - 1]);
    }
  }, [currentStep]);
