...
export default function Funnel2EspPage() {
  ...
  useEffect(() => {
    if (currentStep < 17) {
      ...
      localStorage.setItem('holymind_quiz_state_funnel2_esp', JSON.stringify(stateToSave));
  ...
  useEffect(() => {
    const initializeQuizSession = async () => {
      const savedStateJSON = localStorage.getItem('holymind_quiz_state_funnel2_esp');
      if (savedStateJSON) {
        try {
  ...
  useEffect(() => {
    ...
    if (currentStep <= stepNames.length) {
      trackStepView('funnel2-esp', stepNames[currentStep - 1]);
    }
  }, [currentStep]);
