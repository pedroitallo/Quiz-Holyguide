...
export default function AdminSidebar({ collapsed, onToggle }) {
  ...
  return (
    <aside
      className={`
        fixed left-0 top-0 h-screen bg-black text-white
      ...
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between p-4 border-b border-gray-900">
          ...
          {collapsed && (
            <button
              onClick={onToggle}
              className="absolute top-4 right-2 p-1.5 hover:bg-gray-900 rounded-lg transition-colors"
            >
              ...
              return (
                <li key={item.path}>
                  <Link
                    to={item.path}
                    className={`
                      flex flex-col items-start gap-1 px-3 py-2.5 rounded-lg
